...
class CNN(object):
    def __init__(self, train_data, rng, srng, test_data=[], batch_size = 128, nb_classes = 10, nb_epoch = 10, 
        ...
        self.is_train = T.iscalar('is_train') # pseudo boolean for switching between training and prediction
    ...
    def createCNN(self):
        ...
        layer3 = HiddenLayer(self.rng, self.srng, input=layer3_input, n_in = self.nb_filters[1] * new_rows * new_cols, 
            ...
            is_train = self.is_train, activation = theano.tensor.nnet.relu)        
        ...
        cost = layer5.negative_log_likelihood(self.y)
        ...
        print 'Create theano train/validation functions ...'
        self.train_model = theano.function([self.x, self.y, self.is_train], cost, updates=updates)
        self.validate_model = theano.function([self.x, self.y, self.is_train], layer5.errors(self.y))
        
        print 'Creating theano test data function ...'
        self.test_model = theano.function([self.x, self.is_train], layer5.p_y_given_x)
    ...
    def train(self):
        ...
        while (epoch < self.nb_epoch):
            ...
            for i,minibatch_index in enumerate(xrange(len(X_train)/self.batch_size)):
                ...
                if len(temp_X) == self.batch_size:
                    ...
                    if i == 0: print 'Shape of input:',temp_X.shape
                    cost_ij = self.train_model(temp_X, 
                                               temp_Y,
                                         np.cast['int32'](1))
            validation_losses = []
            for val_batch_idx in xrange(len(X_test)/self.batch_size):
                ...
                if len(temp_X) == self.batch_size:
                    ...
                    validation_losses.append(self.validate_model(temp_X, 
                                                                 temp_Y,
                                                           np.cast['int32'](0)))
...
neural_obj = CNN(train_data, rng, srng, batch_size=64, nb_epoch = 10)
